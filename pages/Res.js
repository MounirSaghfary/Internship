...
const Res = () => {
  ...
  const navigateToTasks = () => {
    console.log(em)
    console.log('jk')
    router.push(`/ResTasks?email=${em}`);
  };

  const navigateToTickets = () => {
    router.push(`/ResTickets?email=${em}`);
  };

  const navigateToPurchases = () => {
    router.push(`/Purchases?email=${em}`);
  };
  ...
  return (
    <div>
        ...
        <div className="flex space-x-4" style={{ paddingTop: 5 }}>
            ...
            <button onClick={navigateToTickets} className="block mt-4 lg:inline-block lg:mt-0 text-black hover:text-gray-300 mr-80 px-4 py-2 rounded" >Tickets</button>
          <Link href="/">
